...
class TrackFrame(ttk.Frame):
    ...
    def sendtoinaturalist(self):
        global inaturalist_token, metadata
        ...
        if inaturalist_token is not None and metadata is not None and len(self.track.inaturalist_id) == 0 and len(self.track.identification) > 0:
            photoset = []
            for b in self.track.blobs:
                if len(b[self.iimagename]) > 0:
                    photoset.append(os.path.join(blobfolder, b[self.iimagename]))
            response = pyinaturalist.create_observation(
                species_guess=self.track.identification,
                observed_on_string=track_datetime(self.track.blobs, self.iimagename),
                time_zone='Sydney',
                ...
                access_token=inaturalist_token,
                photos=photoset
            )

            # Save the new observation ID
            self.track.inaturalist_id = str(response['id'])
            print("New inaturalist record " + self.track.inaturalist_id)
